...
def read_args():
    ...
    parser.add_argument('--sample-chunk-size', type=int, default=512)
    parser.add_argument('--iters', type=int, default=10000)
    parser.add_argument('--workers', '-w', type=int, default=1)
    parser.add_argument('--eval', action='store_true')
    parser.add_argument('--contrastive', action='store_true')
    parser.add_argument('--use-semantic', action='store_true')
    parser.add_argument('--sam-sampling',
                        default='proportional',
                        choices=['proportional', 'uniform', None],
                        type=str,
                        help="SAM sampling method.")
    parser.add_argument('--slow-center', action='store_true')
    parser.add_argument('--cluster-instance-features', action='store_true')
    parser.add_argument(
        '--workspace',
        type=str,
        default=None,
...
def main():
    ...
    dataset = SceneDataset('train',
                           ...
                           load_semantic=flags.use_semantic,
                           sam_sampling=flags.sam_sampling)
    ...
    opt = Namespace(rand_pose=-1,
                    ...
                    feature_constrastive_learning=flags.contrastive,
                    ...
                    contrastive_weight=flags.contrastive_weight,
                    contrastive_temperature=flags.contrastive_temperature,
                    sam_sampling=flags.sam_sampling is not None,
                    slow_center=flags.slow_center)
    ...
    if flags.cluster_instance_features:
        del dataset
        dataset = SceneDataset('test',
                                ...
                                batch_size=8182,
                                lazy=True)
        trainer.compute_instance_centers(dataset)
        trainer.save_instance_centers(save_cluster=True)

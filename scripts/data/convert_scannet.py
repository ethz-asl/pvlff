...
def main():
    ...
    for scene in scenes:
        ...
        instance_dir_in = os.path.join(flags.scannet_scan_dir, scene,
                                       "instance-filt")
        if not os.path.exists(instance_dir_in):
            instance_filt_zip = os.path.join(flags.scannet_scan_dir, scene,
                                             f"{scene}_2d-instance-filt.zip")
            subprocess.call(['unzip', instance_filt_zip, '-d', scene_dir_in])
        ...
        instance_dir = os.path.join(flags.out, scene, "gt_instance")
        ...
        instance_files = os.listdir(instance_dir_in)
        ...
        with SensReader(sensor_file) as reader:
            ...
            stride = max(math.ceil(reader.num_frames / max_frames),
                         flags.stride)
            for i, ((T_WC, rgb, depth), semantic_file,
                    instance_file) in enumerate(
                        zip(reader.read(), semantic_files, instance_files)):
                if i % flags.stride != 0:
                    continue
                ...
                instance_out = os.path.join(instance_dir, f"{number}.png")
                instance_path = os.path.join(instance_dir_in, instance_file)
                instance_frame = cv2.imread(instance_path, -1)
                # Remove instances which belong to classes which are not in the label set.
                # 0 means undefined, and shot not be evaluated on.
                # An object not in the labelset could easily occlude a labelset object.
                instance_frame[out_semantic <= 0] = 0
                cv2.imwrite(instance_out, instance_frame)
        ...
        shutil.rmtree(os.path.join(semantic_dir_in))

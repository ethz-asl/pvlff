...
class ALNetwork(NeRFRenderer):
    ...
    def __init__(self,
        ...
        # instance centers and clusterer
        self.instance_centers = None
        self.instance_clusterer = None
        ...
        self.feature_encoder, self.feature_in_dim = self._get_encoder(encoding)
        ...
        self.contrastive_feat_dim = contrastive_feat_dim
        self.contrastive_features = tcnn.Network(
            n_input_dims=self.feature_in_dim,
    ...
    def forward(self, x, d):
        ...
        x_feat = self.feature_encoder(x, bound=self.bound)
        semantic_features = self.semantic_features(geo_feat)

        contrastive_features = self.contrastive_features(x_feat)
        contrastive_features = F.normalize(contrastive_features)

        return sigma, rgb, semantic_features, contrastive_features
    ...
    def get_params(self, lr):
        params = [{
            ...
            'params': self.contrastive_features.parameters(),
            'lr': lr
        }]
        if self.bg_radius > 0:
    ...
    def semantic(self, geo_features):
        """
    ...
    def contrastive(self, x_feature_encoding, contrastive_ema=None):
        """
        x: [N, 3] points in [-bound, bound]
        returns: [N, C] contrastive features
        """
        if contrastive_ema is not None:
            with contrastive_ema.average_parameters():
                con_features = self.contrastive_features(x_feature_encoding)
        else:
            con_features = self.contrastive_features(x_feature_encoding)
        # con_features = F.normalize(con_features)
        return con_features
    ...
    def network_parameters(self):
        ...
        return (list(self.sigma_net.parameters()) +
                ...
                list(self.contrastive_features.parameters()))
    
    def encoder_parameters(self):
        """
        return: list of parameters in the encoders
        """
        return (list(self.encoder.parameters()) +
                list(self.feature_encoder.parameters()))
    
    def set_instance_centers(self, instance_centers):
        self.instance_centers = instance_centers

    def set_instance_clusterer(self, clusterer):
        self.instance_clusterer = clusterer
